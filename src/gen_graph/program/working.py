            ...
            for person in persons:
                ...
                if maxk > 0:
                    ...
                    if k > 0:
                        ...
                        layer = constants.WORK_2_CLIENT_LAYERS[wt]
                        if math_prob.yes_no(config.CLIENT_IS_EXPORT):
                            env.add_export_edge(constants.TravelType.WORK, person, layer, constants.NO_SUBLAYER, prob,
                                                CalParamIdx.ONE)
                        else:
                            for contact in math_prob.draw_distinct_uniform(k, all_persons_list):
                                if contact is not person:
                                    env.travel_info.add_travel(constants.TravelType.SERVICE, person.town, contact.town,
                                                               contact,
                                                               prob)
                                    env.add_edge(person, contact, layer, constants.NO_SUBLAYER,
                                                 prob, CalParamIdx.ONE)

    for wt in constants.WorkType:
        for person in workers.outofregion[wt]:
            env.add_export_edge(constants.TravelType.WORK, person, constants.Layer.WORK_CONTACTS, constants.NO_SUBLAYER,
                                config.WORK_EXPORT_RATE, CalParamIdx.ONE)
