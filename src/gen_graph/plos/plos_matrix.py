...
class PlosMatrix:
    ...
    def __init__(self, categories: PlosMatrixCats = PlosMatrixAgeCats):
        ...
        self.calibrate = np.zeros([self.size])
        self.persons = set()
    ...
    def add_external(self, age, prob=1):
        self.calibrate[self.categories.value2cat(age)] += prob

    def table(self, size=None):
        with np.errstate(divide='ignore', invalid='ignore'):
            ret = self.sums / self.nums[:, None]
        ret[self.nums == 0, :] = 0
        return (self.coeficients() * ret)[:size, :size]

    def coeficients(self):
        sums = np.sum(self.sums, axis=1)
        with np.errstate(divide='ignore', invalid='ignore'):
            ret = 1 + self.calibrate / sums
            ret[sums == 0] = 1
        return np.atleast_2d(ret).T

    def coeficient(self, row):
        return self.coeficients()[row]

    def __getitem__(self, key):
        n = self.nums[key[0]]
        if not n:
            return 0.0
        return self.coeficient(key[0]) * self.sums[key] / n

    def total(self, row):
        n = self.nums[row]
        if not n:
            return 0.0
        return self.coeficient(row) * np.sum(self.sums[row,]) / n
