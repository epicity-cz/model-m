...
class Renv:
    ...
    def __init__(self, region_config: BaseConfig, towns: List, exports: List[ExportNode],
                 copy_from: Optional[Renv] = None):
        if copy_from:
    ...
    def copy(self):
        cpy = Renv(self.config, self.towns, self.exports, self)
        return cpy
    ...
    def mutual_contacts(self, persons: List, layer, sublayer, w_layer: CalParamIdx, plambda: float = None,
        ...
        if plambda:
            nscale = round(nn * scale)
            if nscale == 0:
                return
            prob *= math_prob.lambdan(plambda, nscale)
    ...
    def add_export_edge(self, type: constants.TravelType, person, layer: constants.Layer, sublayer: int,
                        probability: float, w_layer: CalParamIdx,
                        w_param=None, intensity=None):
        export = math_prob.choice(self.exports)
        ld = constants.LAYER_DEFS[layer]
        if not intensity:
            intensity = self.config.INTENSITIES[ld.default_intensity]
        self.graph.add_link(person, export, layer, sublayer, probability, w_layer, w_param, intensity)
        self.travel_info.add_export_travel(type, person, probability)
