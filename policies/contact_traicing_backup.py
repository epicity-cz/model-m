...
from quarantine_coefs import RISK_FOR_LAYERS, RISK_FOR_LAYERS_MAX, RISK_FOR_LAYERS_MINI, RISK_FOR_LAYERS_60, RISK_FOR_LAYERS_10, RISK_FOR_LAYERS_30
...
class ContactTracingPolicy(TestingPolicy):
        ...
        for i, d_node in enumerate(detected_nodes):
            ...
            nodes_contacts = [
                (contact[1], 
                 ...
                 (self.model.t - days_back +j +1)
                 )
                # five day back
                for j, contact_list in enumerate(self.model.contact_history[-days_back:])
    ...
    def do_contact_tracing(self, detected_nodes):
        ...
        if self.enter_test:
            ...
            self.waiting_room_result_enter_test.lock_up(ill, 2, check_duplicate=True) #todo: take care of duplicates
        ...
        if not self.auto_recover:
            ...
            if len(still_ill) > 0:
                self.model.node_detected[still_ill] = True
            ...
            with_symptoms, without_symptoms = self.symptoms_division(released)
            ready_to_leave = without_symptoms
            self.depo.lock_up(with_symptoms, 1)
    ...
    def symptoms_division(self, nodes):
            all_nodes_symptoms =  (self.model.memberships[STATES.I_s] == 1).ravel()
            nodes_have_symptoms = all_nodes_symptoms[nodes] 
            with_symptoms = nodes[nodes_have_symptoms]
            without_symptoms = nodes[nodes_have_symptoms == False]
            return with_symptoms, without_symptoms
    ...
    def run(self):
        ...
        #todo: do this also in testing
        # reset those who stopped be symptomatic 
...
#            self.riskiness = get_riskiness(0.6, 0.4, 0.1)
            self.riskiness = get_riskiness(0.6, 0.3, 0.1)
