...
class SequentialEngine(SeirsPlusLikeEngine):
    ...
    def inicialization(self):
        ...
        self.testable = np.zeros(shape=(self.graph.number_of_nodes,1), dtype=bool)
...
#        self.states_history[self.t] = self.states_history[self.t-1]
        ...
        for node, idx in enumerate(transition_idx):
            ...
            if idx > len(self.transitions):
                print("DBG WARNING idx from searchsorted on the edge of the array, may be round-off error, better to check ")
                print("DBG WARNING r > sum, but r = np.random.rand() returns [0,1) and sum should be == 1 (except rounding)")
