...
class SEIRSModel():
    ...
    def __init__(self, initN, beta, sigma, gamma, xi=0, mu_I=0, mu_0=0, nu=0, p=0,
        ...
        self.numS = numpy.array([self.N[-1] - self.numE[-1] - self.numI[-1] -
                                 self.numD_E[-1] - self.numD_I[-1] - self.numR[-1] - self.numF[-1]])
        assert(
            self.numS[0] >= 0), "The specified initial population size N must be greater than or equal to the initial compartment counts."
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def system_dfes(t, variables, beta, sigma, gamma, xi, mu_I, mu_0, nu,
        ...
        # varibles is a list with compartment counts as elements
        S, E, I, D_E, D_I, R, F = variables

        N = S + E + I + D_E + D_I + R

        dS = - (beta*S*I)/N - q*(beta_D*S*D_I)/N + xi*R + nu*N - mu_0*S

        dE = (beta*S*I)/N + q*(beta_D*S*D_I)/N - \
            sigma*E - theta_E*psi_E*E - mu_0*E
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run_epoch(self, runtime, dt=0.1):
        ...
        init_cond = [self.numS[-1], self.numE[-1], self.numI[-1],
                     self.numD_E[-1], self.numD_I[-1], self.numR[-1], self.numF[-1]]
        ...
        solution = scipy.integrate.solve_ivp(lambda t, X: SEIRSModel.system_dfes(t, X, self.beta, self.sigma, self.gamma, self.xi, self.mu_I, self.mu_0, self.nu,
                                             ...
                                             t_span=[
                                                 self.t, self.tmax], y0=init_cond, t_eval=t_eval
                                             )
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self, ax=None,  plot_S='line', plot_E='line', plot_I='line', plot_R='line', plot_F='line',
        ...
        Dseries = (self.numD_E+self.numD_I) / \
            self.N if plot_percentages else (self.numD_E+self.numD_I)
        ...
        if(dashed_reference_results):
            ...
            dashedReference_IDEstack = (dashed_reference_results.numI + dashed_reference_results.numD_I + dashed_reference_results.numD_E +
                                        dashed_reference_results.numE)[::int(self.N/100)] / (self.N if plot_percentages else 1)
        ...
        if(shaded_reference_results):
            ...
            shadedReference_IDEstack = (shaded_reference_results.numI + shaded_reference_results.numD_I +
                                        shaded_reference_results.numD_E + shaded_reference_results.numE) / (self.N if plot_percentages else 1)
        ...
        if(any(Fseries) and plot_F == 'stacked'):
            ax.fill_between(numpy.ma.masked_where(Fseries <= 0, self.tseries), numpy.ma.masked_where(
                Fseries <= 0, topstack+Fseries), topstack, color=color_F, alpha=0.5, label='$F$', zorder=2)
            ax.plot(numpy.ma.masked_where(Fseries <= 0, self.tseries), numpy.ma.masked_where(
                Fseries <= 0, topstack+Fseries),           color=color_F, zorder=3)
            topstack = topstack+Fseries
        if(any(Eseries) and plot_E == 'stacked'):
            ax.fill_between(numpy.ma.masked_where(Eseries <= 0, self.tseries), numpy.ma.masked_where(
                Eseries <= 0, topstack+Eseries), topstack, color=color_E, alpha=0.5, label='$E$', zorder=2)
            ax.plot(numpy.ma.masked_where(Eseries <= 0, self.tseries), numpy.ma.masked_where(
        ...
        if(combine_D and plot_D_E == 'stacked' and plot_D_I == 'stacked'):
            ax.fill_between(numpy.ma.masked_where(Dseries <= 0, self.tseries), numpy.ma.masked_where(
                Dseries <= 0, topstack+Dseries), topstack, color=color_D_E, alpha=0.5, label='$D_{all}$', zorder=2)
            ax.plot(numpy.ma.masked_where(Dseries <= 0, self.tseries), numpy.ma.masked_where(
                Dseries <= 0, topstack+Dseries),           color=color_D_E, zorder=3)
            topstack = topstack+Dseries
        else:
            if(any(D_Eseries) and plot_D_E == 'stacked'):
                ax.fill_between(numpy.ma.masked_where(D_Eseries <= 0, self.tseries), numpy.ma.masked_where(
                    D_Eseries <= 0, topstack+D_Eseries), topstack, color=color_D_E, alpha=0.5, label='$D_E$', zorder=2)
                ax.plot(numpy.ma.masked_where(D_Eseries <= 0, self.tseries), numpy.ma.masked_where(
                    D_Eseries <= 0, topstack+D_Eseries),           color=color_D_E, zorder=3)
                topstack = topstack+D_Eseries
            if(any(D_Iseries) and plot_D_I == 'stacked'):
                ax.fill_between(numpy.ma.masked_where(D_Iseries <= 0, self.tseries), numpy.ma.masked_where(
        ...
        if(any(Iseries) and plot_I == 'stacked'):
            ax.fill_between(numpy.ma.masked_where(Iseries <= 0, self.tseries), numpy.ma.masked_where(
                Iseries <= 0, topstack+Iseries), topstack, color=color_I, alpha=0.5, label='$I$', zorder=2)
            ax.plot(numpy.ma.masked_where(Iseries <= 0, self.tseries), numpy.ma.masked_where(
                Iseries <= 0, topstack+Iseries),           color=color_I, zorder=3)
            topstack = topstack+Iseries
        if(any(Rseries) and plot_R == 'stacked'):
            ax.fill_between(numpy.ma.masked_where(Rseries <= 0, self.tseries), numpy.ma.masked_where(
                Rseries <= 0, topstack+Rseries), topstack, color=color_R, alpha=0.5, label='$R$', zorder=2)
            ax.plot(numpy.ma.masked_where(Rseries <= 0, self.tseries), numpy.ma.masked_where(
                Rseries <= 0, topstack+Rseries),           color=color_R, zorder=3)
            topstack = topstack+Rseries
        if(any(Sseries) and plot_S == 'stacked'):
            ax.fill_between(numpy.ma.masked_where(Sseries <= 0, self.tseries), numpy.ma.masked_where(
                Sseries <= 0, topstack+Sseries), topstack, color=color_S, alpha=0.5, label='$S$', zorder=2)
            ax.plot(numpy.ma.masked_where(Sseries <= 0, self.tseries), numpy.ma.masked_where(
...
class SEIRSNetworkModel():
    ...
    def __init__(self, G, beta, sigma, gamma, xi=0, mu_I=0, mu_0=0, nu=0, p=0,
        ...
        # Model Parameters:
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        self.beta = numpy.array(beta).reshape((self.numNodes, 1)) if isinstance(
            beta, (list, numpy.ndarray)) else numpy.full(fill_value=beta, shape=(self.numNodes, 1))
        self.sigma = numpy.array(sigma).reshape((self.numNodes, 1)) if isinstance(
            sigma, (list, numpy.ndarray)) else numpy.full(fill_value=sigma, shape=(self.numNodes, 1))
        self.gamma = numpy.array(gamma).reshape((self.numNodes, 1)) if isinstance(
            gamma, (list, numpy.ndarray)) else numpy.full(fill_value=gamma, shape=(self.numNodes, 1))
        self.xi = numpy.array(xi).reshape((self.numNodes, 1)) if isinstance(
            xi, (list, numpy.ndarray)) else numpy.full(fill_value=xi, shape=(self.numNodes, 1))
        self.mu_I = numpy.array(mu_I).reshape((self.numNodes, 1)) if isinstance(
            mu_I, (list, numpy.ndarray)) else numpy.full(fill_value=mu_I, shape=(self.numNodes, 1))
        self.mu_0 = numpy.array(mu_0).reshape((self.numNodes, 1)) if isinstance(
            mu_0, (list, numpy.ndarray)) else numpy.full(fill_value=mu_0, shape=(self.numNodes, 1))
        self.nu = numpy.array(nu).reshape((self.numNodes, 1)) if isinstance(
            nu, (list, numpy.ndarray)) else numpy.full(fill_value=nu, shape=(self.numNodes, 1))
        self.p = numpy.array(p).reshape((self.numNodes, 1)) if isinstance(
            p, (list, numpy.ndarray)) else numpy.full(fill_value=p, shape=(self.numNodes, 1))

        # Testing-related parameters:
        self.beta_D = (numpy.array(beta_D).reshape((self.numNodes, 1)) if isinstance(beta_D, (list, numpy.ndarray))
                       else numpy.full(fill_value=beta_D, shape=(self.numNodes, 1))) if beta_D is not None else self.beta
        self.sigma_D = (numpy.array(sigma_D).reshape((self.numNodes, 1)) if isinstance(sigma_D, (list, numpy.ndarray))
                        else numpy.full(fill_value=sigma_D, shape=(self.numNodes, 1))) if sigma_D is not None else self.sigma
        self.gamma_D = (numpy.array(gamma_D).reshape((self.numNodes, 1)) if isinstance(gamma_D, (list, numpy.ndarray))
                        else numpy.full(fill_value=gamma_D, shape=(self.numNodes, 1))) if gamma_D is not None else self.gamma
        self.mu_D = (numpy.array(mu_D).reshape((self.numNodes, 1)) if isinstance(mu_D, (list, numpy.ndarray))
                     else numpy.full(fill_value=mu_D, shape=(self.numNodes, 1))) if mu_D is not None else self.mu_I
        self.theta_E = numpy.array(theta_E).reshape((self.numNodes, 1)) if isinstance(
            theta_E, (list, numpy.ndarray)) else numpy.full(fill_value=theta_E, shape=(self.numNodes, 1))
        self.theta_I = numpy.array(theta_I).reshape((self.numNodes, 1)) if isinstance(
            theta_I, (list, numpy.ndarray)) else numpy.full(fill_value=theta_I, shape=(self.numNodes, 1))
        self.phi_E = numpy.array(phi_E).reshape((self.numNodes, 1)) if isinstance(
            phi_E, (list, numpy.ndarray)) else numpy.full(fill_value=phi_E, shape=(self.numNodes, 1))
        self.phi_I = numpy.array(phi_I).reshape((self.numNodes, 1)) if isinstance(
            phi_I, (list, numpy.ndarray)) else numpy.full(fill_value=phi_I, shape=(self.numNodes, 1))
        self.psi_E = numpy.array(psi_E).reshape((self.numNodes, 1)) if isinstance(
            psi_E, (list, numpy.ndarray)) else numpy.full(fill_value=psi_E, shape=(self.numNodes, 1))
        self.psi_I = numpy.array(psi_I).reshape((self.numNodes, 1)) if isinstance(
            psi_I, (list, numpy.ndarray)) else numpy.full(fill_value=psi_I, shape=(self.numNodes, 1))
        self.q = numpy.array(q).reshape((self.numNodes, 1)) if isinstance(
            q, (list, numpy.ndarray)) else numpy.full(fill_value=q, shape=(self.numNodes, 1))
        ...
        self.X = numpy.array([self.S]*int(self.numS[0]) + [self.E]*int(self.numE[0]) + [self.I]*int(self.numI[0]) + [self.D_E]*int(
            self.numD_E[0]) + [self.D_I]*int(self.numD_I[0]) + [self.R]*int(self.numR[0]) + [self.F]*int(self.numF[0])).reshape((self.numNodes, 1))
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def calc_propensities(self):
        ...
        if(numpy.any(self.numI[self.tidx])
            ...
            numContacts_I = numpy.asarray(
                scipy.sparse.csr_matrix.dot(self.A, self.X == self.I))
        ...
        if(self.testing_scenario
            ...
            numQuarantineContacts_DI = numpy.asarray(
        ...
        propensities_StoE = (self.p*((self.beta*self.numI[self.tidx] + self.q*self.beta_D*self.numD_I[self.tidx])/self.N[self.tidx])
                             + (1-self.p)*numpy.divide((self.beta*numContacts_I + self.beta_D*numQuarantineContacts_DI),
                                                       self.degree, out=numpy.zeros_like(self.degree), where=self.degree != 0)
                             )*(self.X == self.S)
        ...
        propensities_EtoDE = (self.theta_E + self.phi_E *
                              numContacts_D)*self.psi_E*(self.X == self.E)
        ...
        propensities_ItoDI = (self.theta_I + self.phi_I *
                              numContacts_D)*self.psi_I*(self.X == self.I)
        ...
        columns = ['StoE', 'EtoI', 'ItoR', 'ItoF', 'EtoDE',
                   'ItoDI', 'DEtoDI', 'DItoR', 'DItoF', 'RtoS', '_toS']
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run_iteration(self):
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        assert(self.X[transitionNode] == self.transitions[transitionType]['currentState'] and self.X[transitionNode] != self.F), "Assertion error: Node " + \
        ...
        self.numS[self.tidx] = numpy.clip(numpy.count_nonzero(
            self.X == self.S), a_min=0, a_max=self.numNodes)
        self.numE[self.tidx] = numpy.clip(numpy.count_nonzero(
            self.X == self.E), a_min=0, a_max=self.numNodes)
        self.numI[self.tidx] = numpy.clip(numpy.count_nonzero(
            self.X == self.I), a_min=0, a_max=self.numNodes)
        self.numD_E[self.tidx] = numpy.clip(numpy.count_nonzero(
            self.X == self.D_E), a_min=0, a_max=self.numNodes)
        self.numD_I[self.tidx] = numpy.clip(numpy.count_nonzero(
            self.X == self.D_I), a_min=0, a_max=self.numNodes)
        self.numR[self.tidx] = numpy.clip(numpy.count_nonzero(
            self.X == self.R), a_min=0, a_max=self.numNodes)
        self.numF[self.tidx] = numpy.clip(numpy.count_nonzero(
            self.X == self.F), a_min=0, a_max=self.numNodes)
        self.N[self.tidx] = numpy.clip((self.numS[self.tidx] + self.numE[self.tidx] + self.numI[self.tidx] +
                                        self.numD_E[self.tidx] + self.numD_I[self.tidx] + self.numR[self.tidx]), a_min=0, a_max=self.numNodes)
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run(self, T, checkpoints=None, print_interval=10, verbose=False):
        ...
        if(checkpoints):
            ...
            checkpointIdx = numpy.searchsorted(checkpoints['t'], self.t)
                        ...
                        if(param in list(checkpoints.keys())):
                            if(param == 'G'):
                                ...
                                setattr(self, param, checkpoints[param][checkpointIdx] if isinstance(checkpoints[param][checkpointIdx], (
                                    list, numpy.ndarray)) else numpy.full(fill_value=checkpoints[param][checkpointIdx], shape=(self.numNodes, 1)))
...
def custom_exponential_graph(base_graph=None, scale=100, min_num_edges=0, m=9, n=None):
    ...
    for node in graph:
        ...
        quarantineEdgeNum = int(max(min(numpy.random.exponential(
